...
test.skip("trip_record takes the traveller from the shared profile, and trip_list totals per currency", async (t) => {
  const box = sandbox();
...
test.skip("trip_export returns expense_add arguments per currency and writes nothing itself", async (t) => {
  const box = sandbox();
...
test.skip("perdiem_report totals per scheme and month and never adds two currencies", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome, key: proKey() });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
...
test.skip("trip_delete refuses a trip with a dependent and names the dependent", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome, key: proKey() });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
