...
test.skip("a corrupt trips.json is quarantined byte for byte and every later call fails", async (t) => {
  const box = sandbox();
...
test.skip("a corrupt counter.json blocks the write and leaves the trips readable", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome, key: proKey() });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
